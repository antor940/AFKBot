function startBotFunctions()
{
    ...
    const { logToLog, logToChat } = require('./Logging');
    
    //Minecraft
    logToLog('<src/BotFunctions.js/Function startBotFunctions> Passed');
    ...
    {
        logToLog('<src/BotFunctions.js/Event on resource_pack_send> Passed');
    ...
    bot.once('health', async () =>
    {
        ...
        logToLog('<src/BotFunctions.js/Event once health> Passed');
    ...
    bot.on('health', async () =>
    {
        if (bot.health < config['low-health']['health-points'] && bot.health > 0)
        {
            ...
            logToLog('<src/BotFunctions.js/Event on health> Passed');
            pauseOnSent = true;
    ...
    bot.on('kicked', async (reason) =>
    {
        if (reason.match(/(banned)/ig))
        {
            ...
            await fieldEmbed('Bot warning', embedArr, '');
            logToLog('<src/BotFunctions.js/Event on kicked> Passed');
    ...
    bot.on('death', async () =>
    {
        ...
        logToLog('<src/BotFunctions.js/Event on death> Passed');
        if (config['windows-notifications']['on-death']) sendNotification('Warning', 'Died or killed');
    });
    
    function notifyUsers()
    {
        logToLog('<src/BotFunctions.js/Function notifyUsers> Passed');
    ...
    function autoLook()
    {
        logToLog('<src/BotFunctions.js/Function autoLook> Passed');
    ...
    function autoPvP()
    {
        logToLog('<src/BotFunctions.js/Function autoPvP> Passed');
        bot.on('entityMoved', (entity) =>
        {
    ...
    function antiKick()
    {
        logToLog('<src/BotFunctions.js/Function antiKick> Passed');
        goRandom();
    ...
    function intervalMessage()
    {
        try
        {
            logToLog('<src/BotFunctions.js/Function intervalMessage> Passed');
            setInterval(() => {
        ...
        {
            logToLog(`<src/BotFunctions.js/ERROR Function intervalMessage> ERROR: ${err}`);
